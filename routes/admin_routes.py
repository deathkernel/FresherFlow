...
def dashboard():
    ...
    stats = {
        ...
        + db.execute("SELECT COUNT(*) FROM external_applications").fetchone()[0],
    }
    ...
    if q:
        job_clauses.append(
            "(v.title LIKE ? OR COALESCE(ep.organization_name, '') LIKE ? OR v.location LIKE ? OR COALESCE(u.email, '') LIKE ?)"
        )
        term = f"%{q}%"
    ...
    applications = db.execute(
        ...
        + " ORDER BY applied_at DESC, id DESC LIMIT 500",
        application_args,
    ).fetchall()
...
def company_detail(user_id):
    db = get_db()
    company = db.execute(
        "SELECT ep.*,u.name contact_name,u.email FROM employer_profiles ep JOIN users u ON u.id=ep.user_id WHERE ep.user_id=?",
        (user_id,),
    ).fetchone()
    if not company:
        return render_template("404.html"), 404
...
def moderate_vacancy(vacancy_id):
    ...
    db.execute(
        "UPDATE vacancies SET moderation_status=?, moderation_note=?, moderated_at=CURRENT_TIMESTAMP, status=? WHERE id=?",
...
def job_detail(job_id):
    db = get_db()
    job = db.execute(
        "SELECT v.*,ep.organization_name,u.email employer_email,(SELECT COUNT(*) FROM applications a WHERE a.vacancy_id=v.id) application_count FROM vacancies v JOIN employer_profiles ep ON ep.user_id=v.employer_id JOIN users u ON u.id=v.employer_id WHERE v.id=?",
        (job_id,),
    ).fetchone()
    if not job:
        return render_template("404.html"), 404
    applications = db.execute(
        "SELECT a.*,u.name,u.email,sp.education,sp.college,sp.skills,sp.resume_filename FROM applications a JOIN users u ON u.id=a.student_id LEFT JOIN student_profiles sp ON sp.user_id=a.student_id WHERE a.vacancy_id=? ORDER BY a.id DESC",
        (job_id,),
    ).fetchall()
    return render_template(
...
def toggle_job(job_id):
    db = get_db()
    job = db.execute(
        "SELECT active FROM external_jobs WHERE id=?", (job_id,)
    ).fetchone()
    if not job:
        return render_template("404.html"), 404
    new_status = 0 if job["active"] else 1
    db.execute(
        "UPDATE external_jobs SET active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        (new_status, job_id),
    )
    db.commit()
...
def sync_jobs():
    ...
    flash(
        (
            f"Sync complete: {inserted} new jobs, {updated - inserted} updated."
            if not errors
            else f"Sync completed with {len(errors)} source error(s)."
        ),
