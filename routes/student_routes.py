...
def valid_resume(filename):return "." in filename and filename.rsplit(".",1)[1].lower() in ALLOWED_RESUME_EXTENSIONS
...
def dashboard():
    ...
    jobs=db.execute("SELECT v.*,ep.organization_name,0 is_external,NULL apply_url FROM vacancies v JOIN employer_profiles ep ON ep.user_id=v.employer_id WHERE v.status='active' AND v.moderation_status='approved' AND ep.account_status='active' ORDER BY v.id DESC LIMIT 6").fetchall()
...
def profile():
    ...
    if request.method=="POST":
        fields=[request.form.get(k,"").strip() for k in ("phone","education","college","graduation_year","skills","certifications","preferred_job_type","preferred_location")];resume=request.files.get("resume");existing=db.execute("SELECT resume_filename FROM student_profiles WHERE user_id=?",(uid,)).fetchone()["resume_filename"];resume_filename=existing
        if resume and resume.filename:
            if not valid_resume(resume.filename):flash("Resume must be a PDF, DOC or DOCX file.","error");return redirect(url_for("student.profile"))
            resume_filename=f"{uid}_{secure_filename(resume.filename)}";Path(current_app.config["UPLOAD_FOLDER"]).mkdir(parents=True,exist_ok=True);resume.save(Path(current_app.config["UPLOAD_FOLDER"])/resume_filename)
        filled=sum(bool(value) for value in fields);strength=min(100,20+filled*10+(10 if resume_filename else 0));db.execute("UPDATE student_profiles SET phone=?,education=?,college=?,graduation_year=?,skills=?,certifications=?,preferred_job_type=?,preferred_location=?,resume_filename=?,profile_strength=? WHERE user_id=?",(*fields,resume_filename,strength,uid));db.commit();flash("Student profile updated successfully.","success");return redirect(url_for("student.profile"))
    profile=db.execute("SELECT * FROM student_profiles WHERE user_id=?",(uid,)).fetchone();user=db.execute("SELECT * FROM users WHERE id=?",(uid,)).fetchone();return render_template("student/profile.html",profile=profile,user=user)
...
def resume():
    ...
    if not profile or not profile["resume_filename"]:return render_template("404.html"),404
...
def jobs():
    ...
    sql="SELECT v.id,v.title,v.vacancy_type,v.description,v.location,v.salary,v.skills,ep.organization_name,EXISTS(SELECT 1 FROM saved_jobs s WHERE s.vacancy_id=v.id AND s.student_id=?) saved,EXISTS(SELECT 1 FROM applications a WHERE a.vacancy_id=v.id AND a.student_id=?) applied,0 is_external,NULL apply_url,NULL source,NULL source_url FROM vacancies v JOIN employer_profiles ep ON ep.user_id=v.employer_id WHERE v.status='active' AND v.moderation_status='approved' AND ep.account_status='active'";args=[uid,uid]
...
def apply_public_job(job_id):
    ...
    try:db.execute("INSERT INTO external_applications(external_job_id,student_id) VALUES(?,?)",(job_id,uid));db.commit();flash("Application recorded in FresherFlow. The external employer has not been contacted automatically.","success")
    except sqlite3.IntegrityError:db.rollback();flash("You have already applied to this public job.","error")
...
def job_details(vacancy_id):
    db=get_db();uid=session["user_id"];job=db.execute("SELECT v.*,ep.organization_name,ep.website,ep.location employer_location FROM vacancies v JOIN employer_profiles ep ON ep.user_id=v.employer_id WHERE v.id=? AND v.status='active' AND v.moderation_status='approved' AND ep.account_status='active'",(vacancy_id,)).fetchone()
    if not job:return render_template("404.html"),404
...
def apply(vacancy_id):
    db=get_db();exists=db.execute("SELECT 1 FROM vacancies v JOIN employer_profiles ep ON ep.user_id=v.employer_id WHERE v.id=? AND v.status='active' AND v.moderation_status='approved' AND ep.account_status='active'",(vacancy_id,)).fetchone()
...
@role_required("student")
def save(vacancy_id):
    db=get_db();uid=session["user_id"];exists=db.execute("SELECT id FROM saved_jobs WHERE vacancy_id=? AND student_id=?",(vacancy_id,uid)).fetchone()
