(() => {
  ...
  skills.forEach((s, i) => {
    ...
    score.setAttribute("y", "12");
    score.textContent = `${s.score}%`;
    g.append(circle, text, score);
  ...
  function position() {
    ...
    svg
      ...
      .forEach((g, i) =>
        g.setAttribute(
          "transform",
          `translate(${px(skills[i].x)} ${py(skills[i].y)})`,
        ),
      );
    linkLayer.querySelectorAll("line").forEach((line) => {
      const a = Number(line.dataset.a),
        b = Number(line.dataset.b);
      line.setAttribute("x1", px(skills[a].x));
      line.setAttribute("y1", py(skills[a].y));
      line.setAttribute("x2", px(skills[b].x));
      line.setAttribute("y2", py(skills[b].y));
    });
  }
  ...
  function select(index) {
    svg
      .querySelectorAll(".skill-node")
      .forEach((g) =>
        g.classList.toggle("selected", Number(g.dataset.skill) === index),
      );
    svg
      .querySelectorAll(".viz-link")
      .forEach((l) =>
        l.classList.toggle(
          "hot",
    ...
    if (label)
      label.textContent = `Focus: ${skills[index].name} · ${skills[index].score}% readiness`;
  }
